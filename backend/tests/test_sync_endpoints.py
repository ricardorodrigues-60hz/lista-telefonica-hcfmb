...
class FakeContato:
    def __init__(self, id, nome, telefone, email, tipo_numero, atualizado_em, excluido=False):
        self.id = str(id)
        self.nome = nome
...
class FakeContatoRepository:
    ...
    def criar_do_offline(self, *, id, nome, telefone, email, tipo_numero, excluido, timestamp, usuario_nome):
        contato = FakeContato(id, nome, telefone, email, tipo_numero, timestamp, excluido)
        self._por_id[str(id)] = contato
        return contato

    def atualizar_do_offline(self, contato, *, nome, telefone, email, tipo_numero, excluido, timestamp, usuario_nome):
        contato.nome = nome
...
def _override_usuario_atual(papel="CONSULTOR"):
    ...
    app.dependency_overrides[get_current_user] = lambda: type(
        "U", (), {"nome": "Tester", "email": "tester@example.com", "papel": papel}
    )()
...
async def test_sync_conflito_aplica_last_write_wins(monkeypatch):
    ...
    timestamp_servidor = datetime.now(timezone.utc).replace(tzinfo=None) - timedelta(hours=1)
    fake_repo._por_id[contato_id] = FakeContato(
        contato_id, "Nome Antigo", "(14) 0000-0000", "antigo@example.com", "publico", timestamp_servidor
    )
...
async def test_sync_conflito_ignora_alteracao_desatualizada(monkeypatch):
    ...
    fake_repo._por_id[contato_id] = FakeContato(
        contato_id, "Nome do Servidor", "(14) 1111-1111", "servidor@example.com", "publico", timestamp_servidor
    )
        ...
        "contatos": [
            {
                ...
                "email": "offline@example.com",
                "tipo_numero": "publico",
                "atualizado_em": (datetime.now(timezone.utc) - timedelta(hours=2)).isoformat(),
                "excluido": False,
            }
        ]
    }
